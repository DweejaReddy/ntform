	...
	while inputs:
		...
		for sc in readable:
			...
			if sc is serverSocket:
				...
				if data:
					...
					try:
						data = eval(str(data))
						message_queues[sc].put(str(data))

						# If socket not in outputs, add it
		...
		for sc in writable:
			if message_queues[sc].empty():
				...
				print(f"[MESSAGE SENT] --> Sending reply to client {sc.getpeername()}: {msg}")
		...
		for sc in exceptional:
			...
			print(f"[ERROR] --> Client {str(sc.getpeername())} connection stopped due to an error")
			sc.close()
			del message_queues[sc]
except Exception as e:
    print(f"[ERROR --> {str(e)}]")
